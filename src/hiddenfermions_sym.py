...
class HiddenFermion(nn.Module):
  ...
  def setup(self):
    ...
    self.orbitals = Orbitals(self.n_elecs,self.n_hid,self.Lx, self.Ly, self.MFinit, self.stop_grad_mf, self.bounds)
    if self.network=="MorenoFFNN":
        self.hidden = [nn.Dense(features=self.features, use_bias=False, param_dtype=jnp.float64) for i in range(self.n_hid)]
        self.output = [nn.Dense(features=(self.n_elecs + self.n_hid), use_bias=True, param_dtype=jnp.float64) for i in range(self.n_hid)]
    elif self.network=="FFNN":
        self.hidden = [nn.Dense(features=self.features,use_bias=False,param_dtype=jnp.float64) for i in range(self.layers)]
        self.output = nn.Dense(features=self.n_hid*(self.n_elecs + self.n_hid),use_bias=True,param_dtype=jnp.float64)
    else:
        raise NotImplementedError()

    if self.MFinit!="random": 
        self.a = self.param('a', zeros, (1,))
        self.b = self.param('b', zeros, (3,)) #needed if we couple two GPUs
...
class Orbitals(nn.Module):
  ...
  def _init_orbitals_dct(self, key, shape):
    ...
    def initialize_obc(num_particles):
      ...
      mat = np.zeros([num_particles,len(rs)])
    ...
    def ft(k_arr, max_val):
      if self.bounds=="OBC" or self.bounds=="PBCOBC" or self.bounds=="OBCPBC":
        matrix = initialize_obc(max_val)
  ...
  def __call__(self,x):
    if self.MFinit=="Fermi":
        orbitals_mfmf = self.param('orbitals_mf',self._init_orbitals_dct,(2*self.Lx*self.Ly,self.n_elecs))
    elif self.MFinit=="Hartree":
        orbitals_mfmf = self.param('orbitals_mf',self._init_orbitals_hartree,(2*self.Lx*self.Ly,self.n_elecs))
    elif self.MFinit=="random":
        orbitals_mfmf = self.param('orbitals_mf', normal(0.1),(2*self.Lx*self.Ly,self.n_elecs))
    else:
        raise NotImplementedError("This MF initialization is not implemented! Chose one of: Fermi, random")
    orbitals_mfhf = self.param('orbitals_hf', zeros,(2*self.Lx*self.Ly,self.n_hid))
